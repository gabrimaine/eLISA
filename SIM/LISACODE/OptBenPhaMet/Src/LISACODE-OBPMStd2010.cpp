...
LCOBPMStd2010::LCOBPMStd2010(LCTools * MT_n)
: LCOBPM(MT_n)
{	
	initNULL(false);
}
...
void LCOBPMStd2010::initNULL(bool CleanMem)
{
...
void LCOBPMStd2010::config(ezxml_t xmlbloc)
{
	ezxml_t param;
	...
	for(param = ezxml_child(xmlbloc,"Param"); param; param = param->next){
		...
		if(MT->wcmp(ezxml_attr(param,"Name"),"OpticalBenchType")){
			...
			if(!MT->wcmp(OpticalBenchType, "Std2010"))
				throw std::invalid_argument("ERROR in LCOBPMStd2010::config : The xml bloc does not correspond to a type of the optical bench + phasemeter 'Std2010' .");
		}
...
void LCOBPMStd2010::config(int iParam, double ParamVal)
{
	
}


void LCOBPMStd2010::LinkNoise(LCNoise ** AllNoises, int NAllNoises)
{
...
void LCOBPMStd2010::LinkFactShotNoise(double * FactShotNoise_n)
{
	if(TypeOBPM == 0)
...
double LCOBPMStd2010::MeasurePho(double TimeLocal, double TShiftEff)
{
...
void LCOBPMStd2010::DispInfo(char * BTab, bool LinkedModule)
{
	if(MT->Disp()){
		if(MT->DispDet()) 
			Cout << Endl << "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << Endl;
...
// end of LISACODE-OBPMStd2010.cpp

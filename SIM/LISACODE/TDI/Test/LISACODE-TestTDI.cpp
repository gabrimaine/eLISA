...
int main (int argc, char * const argv[])
{
	try {
		...
		if(((argc>1)&&(strcmp(argv[1],"--help")==0))||((argc>1)&&(strcmp(argv[1],"-h")==0))){
			...
			Coutm << "\t\t(./)LC2TestTDI [Options] " << Endl;
			Coutm << Endl << "\tArguments :" << Endl;
			Coutm << "\t\t * %sfOut       : Output file name : t hBp1 hBc1 hBp2 hBc2 ... [default: TestTDI]. " << Endl;
			Coutm << "\t\t * %sfInXMLFile : XML input file [default: TestTDI.xml]. " << Endl;
		...
		if(((argc>1)&&(strcmp(argv[1],"--version")==0))&&((argc>1)&&(strcmp(argv[1],"-v")==0))){
			Coutm << " ----- VERSION -----" << Endl;
			Coutm << " LC2TestTDI : executable testing tdi - LISACode package - version " << LC::LCVersion << " at " << LC::DateOfLastUpdate << Endl;
			Coutm << " ----------------" << Endl;
			return 0;
		}
		...
		std::cerr << "TestTDI: error: " << e.what()<<Endl;
		std::cerr << "TestTDI: abort!" << Endl;
		exit(1);
	}
	return(0);
};
